...
- **경로 B의 실제 LLM 실측이 없다.** 지금까지 확인한 것은 로직(근거 대조·어휘
  검증·결측 처리)이 의도대로 동작한다는 것까지다. GPU 장비에서 실물 서류로
  **재현율**과 **근거 통과율** 두 숫자를 재야 한다 — 근거 통과율이 낮으면 프롬프트
  문제이고, 높은데 재현율이 낮으면 OCR 품질 문제다
