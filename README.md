...
- `audio_preprocess.py` — STT 호출 직전 오디오 신호를 다듬는 전처리 유틸리티(소음 제거, 진폭 정규화, 고주파 강조). 의료 용어가 작은 목소리·배경 소음에 묻혀 STT가 뭉개는 문제를 모델 교체 없이 완화하기 위한 것으로, `transcribe.py`/`call_capture.py` 양쪽에서 재사용된다.
...
- `mic_recorder.py` — 마이크 입력을 numpy 버퍼에 누적하고 WAV로 저장하는 모듈. `sounddevice` + `soundfile` 기반으로 16kHz 모노 녹음 지원. 마이크 권한 없음/장치 없음/저장 실패 시 사용자에게 원인을 알려주는 예외 처리 포함. Step 1 스모크 테스트(`record_fixed_seconds()`) 포함.
- `call_capture.py` — **실제 운영 흐름(통화 종료 후 배치 처리)에 대응하는 진입점.** 마이크로 "통화 시작"을 흉내내고, 통화 중에는 녹음만 계속 누적한다(중간에 STT를 돌리지 않음). Ctrl+C를 "통화 종료"로 취급해 그 순간 전체 녹음을 WAV로 저장한 뒤, `transcribe.py`의 `transcribe()`를 그대로 호출해 STT → 필터링 → SBAR 구조화까지 자동으로 이어서 실행한다. 새 로직을 만들지 않고 배치 파이프라인을 그대로 재사용하므로, 결과 품질·포맷은 사전 녹음 파일을 처리할 때와 동일하다.
- `live_transcribe.py` — 마이크를 백그라운드에서 계속 녹음하면서 N초마다 누적 버퍼 전체를 Whisper에 재변환해, 통화 도중에도 텍스트가 실시간으로 갱신되는 것처럼 보이게 하는 실험적 오케스트레이터. Whisper가 매번 처음부터 전체를 다시 변환하는 구조라 사이클 경계에서 같은 발화가 다른 텍스트로 중복 출력될 수 있다는 한계가 있다 (아래 "알려진 제약사항" 참고). 통화 중 실시간 자막이 실제로 필요한 경우가 아니라면 `call_capture.py` 사용을 권장한다.

> `transcribe.py`는 로컬 파일 기반이고, `call_capture.py`/`live_transcribe.py`는 마이크 기반이다. 셋 다 배치 API인 Whisper를 그대로 쓰되, `call_capture.py`는 "통화 시작~종료를 녹음해 파일 하나로 만든 뒤 배치 파이프라인에 넣는다"는 실제 운영 흐름과 동일한 방식이고, `live_transcribe.py`는 그와 별개로 "라이브처럼 보이는" 실험적 구현(주기적 재변환)이다. 출력 포맷은 셋 다 아래 "입출력 데이터 포맷"에 정의된 동일한 JSON 스키마를 따른다.
...
| 구분        | 모델명                                                                                                                                                                                                               | 용도                               | 비고                                      |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- | --------------------------------------- |
| STT       | Whisper medium (CTranslate2 변환판, faster-whisper) — [Systran/faster-whisper-medium](https://huggingface.co/Systran/faster-whisper-medium)                                                                | 음성 → 텍스트 변환                      | 기본값은 `medium`(정확도·메모리 절충). `--model` 옵션으로 `large-v3` 등으로 변경 가능                 |
...
- **필터링 자체보다 그 앞 단계인 STT 정확도가 병목이다.** Whisper는 일상 대화 학습 비중이
높아 "산소포화도", "심정지" 같은 한국어 의료 용어를 오인식하는 경우가 실제로 관찰됨
(예: "심정지가 왔었습니다" → "심정도 너무 왔었습니다"). 필터링·구조화는 입력받은 텍스트를
그대로 신뢰하므로, STT가 틀리면 이후 단계도 같이 틀린다. `audio_preprocess.py`로 저비용
개선을 시도했지만 근본 해결에는 응급실 도메인 데이터로 파인튜닝이 필요할 수 있음.
...
| `summary.severity_tag`              | `"high"`          | `"medium"`                                                       |
| `summary.required_department`       | string            | null                                                             |
...
| 옵션               | 기본값         | 설명                                                              |
| ---------------- | ----------- | --------------------------------------------------------------- |
| `--model`        | `medium`  | Whisper 모델 크기 (`tiny`, `base`, `small`, `medium`, `large-v3` 등) |
| `--language`     | `ko`        | 언어 코드                                                           |
...
마이크 권한 요청이 나면 시스템 설정 > 개인정보 보호 > 마이크에서 터미널 앱에 권한을 부여해야 한다 (macOS).

**3-2. 통화 캡처 → 종료 시 자동 파이프라인 실행 (권장)**

실제 운영 흐름("전화가 시작되고 끝나면, 그 통화 전체가 하나의 음성 파일이 되어 STT
파이프라인에 들어간다")을 그대로 구현한 진입점이다. 통화 중에는 녹음만 하고, Ctrl+C를
누른 시점(통화 종료)에 한 번만 전체 오디오를 STT에 넣는다 — `live_transcribe.py`처럼
N초마다 끊어서 재변환하지 않는다.
...
`transcribe.py`의 `transcribe()`를 그대로 호출하므로, 녹음 파일이 `data/voice_data/origin_data/`에
저장되고 이후 산출물 위치·JSON 형식은 사전 녹음 파일을 배치 처리할 때와 동일하다 (아래
"산출물" 표는 `transcribe.py` 섹션 참고).

| 옵션               | 기본값         | 설명                                          |
| ----------------- | ----------- | ------------------------------------------- |
| `--session`       | (자동 생성)     | 세션(파일) 이름 (지정 안 하면 `YYYY_MMDD_HHMM` 형식으로 자동 생성) |
...
**3-3. (실험적) 라이브 캡처 → 주기적 재변환 → 실시간 텍스트 추출**

> ⚠️ **Whisper는 진정한 스트리밍 STT가 아니다.** 다음은 통화 중에도 텍스트가 실시간으로
갱신되는 것처럼 "라이브처럼 보이게" 만든 실험적 구현이다: 마이크를 계속 녹음하면서 N초마다
누적 버퍼 전체를 Whisper에 다시 넣어 재변환하고, 새로 나온 세그먼트만 출력한다. 이 방식의
특성상 재변환 사이클마다 세그먼트 경계가 흔들려 같은 발화가 다른 텍스트로 중복 출력될 수
있다 (아래 "알려진 제약사항" 참고). 통화 중 실시간 자막이 실제로 필요한 경우가 아니라면
3-2의 `call_capture.py`를 쓰는 게 맞다.

```bash
python voice/live_transcribe.py --model medium --stt-interval 5 --sbar-interval 50
```

마이크에서 계속 녹음하면서 5초마다 누적 버퍼 전체를 Whisper에 재변환한다:
...
Ctrl+C로 중지하면, 마지막 재변환 주기 이후 아직 STT를 거치지 않은 구간을 종료 직전
한 번 더 재변환해 텍스트 누락을 방지한 뒤 최종 요약을 생성한다:
...
| 산출물           | 저장 위치                                                  | 설명                                      |
| ------------- | ------------------------------------------------------ | --------------------------------------- |
| 전체 라이브 녹음 WAV | `data/voice_data/live_audio/<세션명>.wav`                 | 마이크로 녹음한 전체 오디오 (인터벌 조각들을 누적한 것)        |
| 누적 STT 텍스트    | `data/voice_data/live_text/<세션명>.txt`                  | 매 재변환 사이클마다 업데이트되는 텍스트 (새 세그먼트만 append) |
| SBAR 구조화 JSON | `data/voice_data/summary_text/<세션명>_call_summary.json` | 주기적(기본 20초)으로 갱신되며, Ctrl+C 후 최종 1회 생성   |



| 옵션                | 기본값         | 설명                                          |
| ----------------- | ----------- | ------------------------------------------- |
| `--session`       | (자동 생성)     | 세션 이름 (지정 안 하면 `YYYY_MMDD_HHMM` 형식으로 자동 생성) |
| `--model`         | `medium`    | Whisper 모델 크기                               |
...
- `call_capture.py`(3-2)는 통화 종료 시점에 배치 파이프라인을 그대로 한 번 호출하는 방식이라
가장 단순하고, 실제 운영 흐름과 정확히 대응한다. 일반적으로는 이쪽을 쓰면 된다.
- `live_transcribe.py`(3-3)는 매 사이클 누적된 모든 턴에 대해 `MedicalRelevanceFilter.classify_turns()`를
재실행해서 의료 관련성을 실시간으로 판정하고, SBAR JSON도 주기마다 파일에 덮어쓴다 —
통화 중 실시간 자막이 실제로 필요할 때만 쓴다.
- 마이크 권한, Ollama 자동 부트스트래핑 등은 배치 처리와 동일하게 자동으로 처리된다
- 실제 구급대원 통화 시나리오를 시뮬레이션하려면 `.docs/voice-live-getting-started.md`를 참고 (Step 1~5 상세 가이드)
...
모든 파이썬 코드는 `voice/` 폴더에 있으므로 상호 import(`from filtering import ...`)는 그대로 동작하며, 실행은 저장소 루트에서 `python voice/transcribe.py ...` 형태로 한다.

모든 데이터는 `data/voice_data/` 하위에 조직되어, 각 기능 모듈이 서로 다른 데이터 폴더를 가질 수 있도록 확장 가능한 구조다.
...
- `data/voice_data/` 폴더(그 하위 `origin_data/`·`origin_noise_data/`·`origin_text/`·`summary_text/`·`live_audio/`·`live_text/` 전부 포함)는 `.gitignore`에 포함되어 있어 오디오 원본과 변환 결과물은 저장소에 올라가지 않음
...
- `audio_preprocess.py`의 소음 제거/정규화/고주파 강조 효과는 아직 실제 통화 샘플로 정량 검증되지 않음 (랜덤 신호로 예외 없이 동작하는 것만 확인한 상태)
- `call_capture.py` 관련 제약사항:
...
- (실험적) 라이브 캡처(`live_transcribe.py`) 관련 제약사항:
  - Whisper는 진정한 스트리밍 STT가 아니므로, 재변환 대상 길이가 늘어날수록 처리 비용이 증가함. 장기 녹음 시 느려질 수 있으니 모델 크기(`--model base` 등) 조정 필요
  - **재변환 사이클마다 세그먼트 경계(VAD 판정)가 다시 흔들려서, 같은 발화가 서로 다른 텍스트로 중복 출력되는 문제가 실제로 관찰됨** (예: "구급대원입니다"가 한 사이클엔 "구급대현입니다", 다음 사이클엔 "구구 대원입니다"로 다르게 인식되어 둘 다 텍스트에 남음) — 근본 해결에는 진짜 스트리밍 STT(예: whisper_streaming의 LocalAgreement 방식)로 교체 필요. 실시간 자막이 필수가 아니라면 `call_capture.py` 사용을 권장하는 이유이기도 함
  - 마이크 권한 설정(macOS 시스템 설정 > 개인정보 보호 > 마이크) 필수
