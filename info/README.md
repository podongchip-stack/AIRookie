...
| 발견 | 무슨 일이 있었나 |
|---|---|
| **음수 병상 = 과밀** | `hvec`에 음수가 온다. "정원보다 환자가 그만큼 많다"는 뜻인데, 매퍼가 `-1` 이하를 전부 미상 처리해 **가장 갈 수 없는 병원이 "모르는 병원"이 되고 있었다.** 서울 55곳 중 6곳이 여기 해당했고 전부 상급종합병원이다. (최초 실측에서는 `-24 ~ -2`로 적었으나, 전국 34일치로 넓힌 결과 실제 범위는 **`-47 ~ 33`**이다 — 아래 "과밀은 순간이 아니라 상태다" 참고) |
...
### 과밀은 순간이 아니라 상태다 (2026-09-14 · 전국 34일치)

위 실측은 **한 시점 한 번**(서울 55곳)이라, 과밀을 "그 순간 붐비는 상태"로 읽었다.
`snapshot.py`가 쌓아둔 34일치(854관측 · 313,371칸)로 다시 보니 그 해석이 틀렸다.
재현은 `python -m hospital_score.report`의 3절(API 호출 0회).

| 관측 | 값 |
|---|---|
| 과밀(`hvec <= -2`) | 6,896칸 / 311,176칸 (**2.22%**) |
| 한 번이라도 과밀이었던 병원 | **127곳** |
| 과밀 구간 | 897개 (완결 233 / 절단 664) |
| 최장 기록 | **삼성서울병원 3일 연속** (274회 연속 관측) |
| 최대 깊이 | **`-47`** (분당서울대). `-40` 이하 70건 = 분당서울대 39 + 삼성서울 31 |

- **최장 10건이 전부 수도권 상급종합병원이다** — 삼성서울 · 분당서울대 · 서울대 ·
  서울아산 · 서울성모. 과밀은 흩어진 현상이 아니라 소수 병원에 **고착**돼 있다
- **분당서울대는 712회 관측 중 688회(96.6%)가 음수**이고 최고값이 `4`다. 이 병원은
  자기 신고 기준으로 응급실 병상이 사실상 상시 없다. "지금 붐빔"이 아니라 상태다
- 완결 구간 지속시간 중앙값은 20분인데 이건 **수집 주기 그 자체**라 해상도에 눌린
  값이다(233개 중 94개가 단발 관측). 인용하려면 최장 기록과 2.22% 쪽을 쓸 것
- 절단 664개 중 191개는 `hvec`가 `-1`로 바뀌어 끊겼다. 즉 **하나의 과밀 에피소드가
  `-1` 하나에 여러 조각으로 쪼개진다** — "3일"조차 하한이다

**그래서 무엇이 문제인가.** 이 6,896칸은 hub로 나갈 때 `clamp_available()`이
전부 `0`으로 낮춘다(스키마가 음수를 막는다). 정원을 47명 초과해 복도까지 찬 병원과
딱 만실인 병원이 dashboard에서 같은 "0병상"으로 보인다는 뜻이다. 우리는 **미상과
확인된 만실**은 `bedCountUnknown`으로 집요하게 구분해 왔는데, **만실과 과밀**은
구분하지 않고 있다. 같은 사상을 한 칸 더 밀면 나오는 축이다.
