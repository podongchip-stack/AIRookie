...
**LLM 실측은 했다** (2026-08-04, 합성 서류 53건·81페이지, `qwen3:14b`).
근거 통과율 **72.4%**(267/369)가 나왔고, 기각 102건의 최대 원인은 환각이 아니라
**근거 인용 규율**이었다 — 63건 중 37건이 프롬프트의 코드·한글 라벨을 근거란에
복사한 것이었다. 즉 값은 맞는데 근거 때문에 버려진 경우라, 환각을 막은 게 아니라
재현율을 잃었다. 자세한 숫자는 [`../README.md`](../README.md) "필드 추출 실측" 참고.

그래서 대응은 모델이 아니라 프롬프트였고, 지시문은 이미 고쳤다.
**그런데 고친 뒤 재측정을 하지 않았다** — 파인튜닝을 검토하기 전에 재측정이 먼저다.

아직 안 잰 것은 둘이다.

- **재현율** — 사람이 읽은 정답 대비 몇 개를 뽑는가. 정답지(`LLMdata/07_eval_gold.jsonl`)를
  손으로 채워야 셀 수 있다
- **실제 병원 서류에서의 성능** — 위 실측은 전부 **합성 서류**다. 기관명도 내용도
  진짜가 아니라, 실물 서식·표기·인쇄 품질에서 같은 숫자가 나온다는 보장이 없다
...
2. **`HospitalInfo` 병합** — 서류에서 뽑은 필드를 E-Gen 실시간 데이터와 합친다.
   서류는 정적이라 좌표·실시간 병상 수는 건드리지 않고, 공개 API가 못 채우는 것만
   덮는다. 어느 값이 어디서 왔는지(AI/규칙) 표기하는 방식은 미정

   > **메워야 할 공백이 좁아졌다 (2026-08-12).** 진료과별 전문의 수는 **심평원
   > 의료기관별상세정보서비스**가 주는 것으로 확인돼 이미 연동했다(병원 518곳,
   > [`Hospital_inform/info/hospital_score/`](../Hospital_inform/info/hospital_score/README.md)).
   > 따라서 서류로만 채울 수 있는 것은 **tPA(`PROC_IV_THROMBOLYSIS`)와 당직 편성**
   > 두 가지다 — tPA는 `MKioskTy` 28항목 어디에도 없고, "그 과에 오늘 밤 누가 있는가"는
   > 어느 공개 API에도 없다. 병합 설계에서 우선순위를 이 둘에 둔다
3. **feature/hub 로 전송**
